  ...
  const rejectionPreview = !hasSlot
    ...
    : !enoughCapacity
      ...
      : !enoughDepth
        ...
        : null;
  const willFill = !toggledAny && !rejectionPreview && sizeNum > 0;

  async function handleSwap() {
    if (pending || sizeNum <= 0) return;
    setPending(true);
    setResult(null);
    try {
      ...
      if (outcome === "filled") {
        const out = quoteSwapOutUsdc(s.pool, sizeNum);
        ...
        <dl className="space-y-1.5 rounded-md border border-border bg-surface-raised/50 p-4 font-mono text-sm">
          <div className="flex items-baseline justify-between">
            ...
            <dd className="text-lg font-medium tabular-nums text-foreground">
              {sizeNum > 0 ? `${fmtUsdc(outUsdc)} USDC` : "—"}
            </dd>
          </div>
          <div className="flex items-baseline justify-between">
            ...
            <dd className="tabular-nums text-foreground">
              ${fmtUsdc(execPrice, 2)}{" "}
              <span className="text-muted">/ spot ${fmtUsdc(spot, 2)}</span>
            </dd>
          </div>
          <div className="flex items-baseline justify-between">
            <dt className="text-muted">price impact</dt>
            <dd className="tabular-nums text-foreground">
              {sizeNum > 0 ? `${(impact * 100).toFixed(2)}%` : "—"}
            </dd>
          </div>
